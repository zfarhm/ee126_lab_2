...
uint32_t MemoryHierarchy::find_in_main_memory_word(uint32_t addr){

	// like we did in the other function
	uint32_t index = addr / WORD_SIZE;
...
int main(int argc, char **argv) {
	...
	if (!(L1_total_hits == 0)){
		...
		// miss_rate_L1 = L1Cache.getReadMissRate();
	}

	if (!(L2_total_hits == 0 )){
		miss_rate_L2 = ( L2_misses / L2_total_hits );
		// miss_rate_L2 = L2Cache.getReadMissRate();
	}
